...
Milestone 01 — Audio output

## User story

As a **developer/PO**, I want **enginehost lifecycle (start/stop audio)** so the product stays shippable and verifiable on device.

## Goal

Ship **EngineHost lifecycle (start/stop audio)** as a complete vertical slice — demoable once on Android without follow-up fixes (M01).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-01-03
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **enginehost lifecycle (start/stop audio)** applies.
...
- End-to-end wiring for **EngineHost lifecycle (start/stop audio)** (Flutter → bridge → C++ engine where applicable)
- Serialization round-trip when state is persisted (M01 save/load rules)
...
1. Launch app → exercise **enginehost lifecycle (start/stop audio)** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **enginehost lifecycle (start/stop audio)** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-01-03-ux-ui.md)
- [Interaction](US-01-03-interaction.md)
