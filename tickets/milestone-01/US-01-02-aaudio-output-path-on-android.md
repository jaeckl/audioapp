...
As a **user**, I want **aaudio output path on android** so my workflow on phone feels like a real DAW, not a demo.

## Goal

Ship **AAudio output path on Android** as a complete vertical slice — demoable once on Android without follow-up fixes (M01).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-01-02
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **aaudio output path on android** applies.
...
- End-to-end wiring for **AAudio output path on Android** (Flutter → bridge → C++ engine where applicable)
...
1. Launch app → exercise **aaudio output path on android** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **aaudio output path on android** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-01-02-ux-ui.md)
- [Interaction](US-01-02-interaction.md)
