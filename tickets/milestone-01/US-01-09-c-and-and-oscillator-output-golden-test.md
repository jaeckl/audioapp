...
As a **developer/PO**, I want **c++ oscillator output golden test** so the product stays shippable and verifiable on device.

## Goal

Ship **C++ oscillator output golden test** as a complete vertical slice — demoable once on Android without follow-up fixes (M01).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-01-09
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **c++ oscillator output golden test** applies.
...
- End-to-end wiring for **C++ oscillator output golden test** (Flutter → bridge → C++ engine where applicable)
...
1. Launch app → exercise **c++ oscillator output golden test** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **c++ oscillator output golden test** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-01-09-ux-ui.md)
- [Interaction](US-01-09-interaction.md)
