...
As a **user**, I want **transport ui toggles play state** so my workflow on phone feels like a real DAW, not a demo.

## Goal

Ship **Transport UI toggles play state** as a complete vertical slice — demoable once on Android without follow-up fixes (M01).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-01-06
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **transport ui toggles play state** applies.
...
- End-to-end wiring for **Transport UI toggles play state** (Flutter → bridge → C++ engine where applicable)
...
1. Launch app → exercise **transport ui toggles play state** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **transport ui toggles play state** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-01-06-ux-ui.md)
- [Interaction](US-01-06-interaction.md)
