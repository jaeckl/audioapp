...
As a **user**, I want **silence when transport stopped** so my workflow on phone feels like a real DAW, not a demo.

## Goal

Ship **Silence when transport stopped** as a complete vertical slice — demoable once on Android without follow-up fixes (M01).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-01-08
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **silence when transport stopped** applies.
...
- End-to-end wiring for **Silence when transport stopped** (Flutter → bridge → C++ engine where applicable)
...
1. Launch app → exercise **silence when transport stopped** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **silence when transport stopped** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-01-08-ux-ui.md)
- [Interaction](US-01-08-interaction.md)
