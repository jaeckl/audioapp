# US-00-09: Transport bar chrome (BPM, playhead label, version)
...
As a **user**, I want **transport bar chrome (bpm, playhead label, version)** so my workflow on phone feels like a real DAW, not a demo.

## Goal

Ship **Transport bar chrome (BPM, playhead label, version)** as a complete vertical slice — demoable once on Android without follow-up fixes (M00).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-00-09
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **transport bar chrome (bpm, playhead label, version)** applies.
...
- End-to-end wiring for **Transport bar chrome (BPM, playhead label, version)** (Flutter → bridge → C++ engine where applicable)
...
1. Launch app → exercise **transport bar chrome (bpm, playhead label, version)** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **transport bar chrome (bpm, playhead label, version)** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-00-09-ux-ui.md)
- [Interaction](US-00-09-interaction.md)
