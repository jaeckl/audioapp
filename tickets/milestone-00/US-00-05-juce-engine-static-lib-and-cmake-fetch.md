...
As a **developer/PO**, I want **juce engine static lib & cmake fetch** so the product stays shippable and verifiable on device.

## Goal

Ship **JUCE engine static lib & CMake fetch** as a complete vertical slice — demoable once on Android without follow-up fixes (M00).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-00-05
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **juce engine static lib & cmake fetch** applies.
...
- End-to-end wiring for **JUCE engine static lib & CMake fetch** (Flutter → bridge → C++ engine where applicable)
...
1. Launch app → exercise **juce engine static lib & cmake fetch** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **juce engine static lib & cmake fetch** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-00-05-ux-ui.md)
- [Interaction](US-00-05-interaction.md)
