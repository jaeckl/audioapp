# US-02-08: Empty device strip hint when no selection
...
As a **user**, I want **empty device strip hint when no selection** so my workflow on phone feels like a real DAW, not a demo.

## Goal

Ship **Empty device strip hint when no selection** as a complete vertical slice — demoable once on Android without follow-up fixes (M02).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-02-08
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **empty device strip hint when no selection** applies.
...
- End-to-end wiring for **Empty device strip hint when no selection** (Flutter → bridge → C++ engine where applicable)
...
1. Launch app → exercise **empty device strip hint when no selection** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **empty device strip hint when no selection** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-02-08-ux-ui.md)
- [Interaction](US-02-08-interaction.md)
