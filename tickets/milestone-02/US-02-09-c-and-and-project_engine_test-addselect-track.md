# US-02-09: C++ project_engine_test (add/select track)
...
As a **developer/PO**, I want **c++ project_engine_test (add/select track)** so the product stays shippable and verifiable on device.

## Goal

Ship **C++ project_engine_test (add/select track)** as a complete vertical slice — demoable once on Android without follow-up fixes (M02).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-02-09
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **c++ project_engine_test (add/select track)** applies.
...
- End-to-end wiring for **C++ project_engine_test (add/select track)** (Flutter → bridge → C++ engine where applicable)
...
1. Launch app → exercise **c++ project_engine_test (add/select track)** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **c++ project_engine_test (add/select track)** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-02-09-ux-ui.md)
- [Interaction](US-02-09-interaction.md)
