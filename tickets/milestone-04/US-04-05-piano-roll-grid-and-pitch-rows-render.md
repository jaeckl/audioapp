# US-04-05: Piano roll grid & pitch rows render
...
As a **user**, I want **piano roll grid & pitch rows render** so my workflow on phone feels like a real DAW, not a demo.

## Goal

Ship **Piano roll grid & pitch rows render** as a complete vertical slice — demoable once on Android without follow-up fixes (M04).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-04-05
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **piano roll grid & pitch rows render** applies.
...
- End-to-end wiring for **Piano roll grid & pitch rows render** (Flutter → bridge → C++ engine where applicable)
...
1. Launch app → exercise **piano roll grid & pitch rows render** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **piano roll grid & pitch rows render** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-04-05-ux-ui.md)
- [Interaction](US-04-05-interaction.md)
