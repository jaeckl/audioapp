...
As a **user**, I want **setmidiclipnotes bridge round-trip** so my workflow on phone feels like a real DAW, not a demo.

## Goal

Ship **setMidiClipNotes bridge round-trip** as a complete vertical slice — demoable once on Android without follow-up fixes (M04).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-04-04
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **setmidiclipnotes bridge round-trip** applies.
...
- End-to-end wiring for **setMidiClipNotes bridge round-trip** (Flutter → bridge → C++ engine where applicable)
...
1. Launch app → exercise **setmidiclipnotes bridge round-trip** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **setmidiclipnotes bridge round-trip** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-04-04-ux-ui.md)
- [Interaction](US-04-04-interaction.md)
