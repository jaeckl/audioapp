# US-03-04: Clip start beat & length in project model
...
Milestone 03 — Clips & transport

## User story

As a **user**, I want **clip start beat & length in project model** so my workflow on phone feels like a real DAW, not a demo.

## Goal

Ship **Clip start beat & length in project model** as a complete vertical slice — demoable once on Android without follow-up fixes (M03).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-03-04
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **clip start beat & length in project model** applies.
...
- End-to-end wiring for **Clip start beat & length in project model** (Flutter → bridge → C++ engine where applicable)
- Serialization round-trip when state is persisted (M03 save/load rules)
...
1. Launch app → exercise **clip start beat & length in project model** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **clip start beat & length in project model** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-03-04-ux-ui.md)
- [Interaction](US-03-04-interaction.md)
