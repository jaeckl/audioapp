...
As a **user**, I want **zip archive packs json + sample refs** so my workflow on phone feels like a real DAW, not a demo.

## Goal

Ship **Zip archive packs JSON + sample refs** as a complete vertical slice — demoable once on Android without follow-up fixes (M05).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-05-04
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **zip archive packs json + sample refs** applies.
...
- End-to-end wiring for **Zip archive packs JSON + sample refs** (Flutter → bridge → C++ engine where applicable)
...
1. Launch app → exercise **zip archive packs json + sample refs** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **zip archive packs json + sample refs** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-05-04-ux-ui.md)
- [Interaction](US-05-04-interaction.md)
