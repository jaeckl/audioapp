# US-05-03: project.json schema v1 via juce::JSON
...
As a **user**, I want **project.json schema v1 via juce::json** so my workflow on phone feels like a real DAW, not a demo.

## Goal

Ship **project.json schema v1 via juce::JSON** as a complete vertical slice — demoable once on Android without follow-up fixes (M05).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-05-03
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **project.json schema v1 via juce::json** applies.
...
- End-to-end wiring for **project.json schema v1 via juce::JSON** (Flutter → bridge → C++ engine where applicable)
...
1. Launch app → exercise **project.json schema v1 via juce::json** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **project.json schema v1 via juce::json** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-05-03-ux-ui.md)
- [Interaction](US-05-03-interaction.md)
