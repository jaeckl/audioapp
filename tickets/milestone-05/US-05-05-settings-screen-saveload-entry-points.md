# US-05-05: Settings screen save/load entry points
...
As a **user**, I want **settings screen save/load entry points** so my workflow on phone feels like a real DAW, not a demo.

## Goal

Ship **Settings screen save/load entry points** as a complete vertical slice — demoable once on Android without follow-up fixes (M05).

## Background

- [roadmap.md](../../docs/milestones/roadmap.md) — US-05-05
- [AGENT.md](../../AGENT.md) §2.5–2.7 — vertical slice, JUCE JSON, system dialogs
- Relevant architecture docs under `docs/architecture/`

## UX flow

1. User opens the DAW and reaches the surface where **settings screen save/load entry points** applies.
...
- End-to-end wiring for **Settings screen save/load entry points** (Flutter → bridge → C++ engine where applicable)
...
1. Launch app → exercise **settings screen save/load entry points** per UX flow above.\n2. Verify success feedback and persistence if applicable.
...
User can **settings screen save/load entry points** with immediate feedback — the milestone “wow moment” for this slice.
...
- [UX/UI](US-05-05-ux-ui.md)
- [Interaction](US-05-05-interaction.md)
