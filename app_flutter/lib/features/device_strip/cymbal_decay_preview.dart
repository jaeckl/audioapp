...
/// Shimmering decay preview for the cymbal generator strip.
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var band = 0; band < 6; band++) {
      ...
      for (var i = 0; i <= steps; i++) {
        ...
        final shimmer = math.sin(t * (8 + metal.clamp(0.0, 1.0) * 12 + band)) * 2;
        final amp = math.exp(-t / ampTau) * (0.15 + brightness.clamp(0.0, 1.0) * 0.35);
...
String cymbalDecayLabel(double norm) {
  final sec = (0.4 + (1 - norm.clamp(0.0, 1.0)) * 2.6);
