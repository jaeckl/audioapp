part of 'snare_envelope_preview.dart';
...
class _SnareEnvelopePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i <= steps; i++) {
      ...
      final bodyEnv =
          math.exp(-t / bodyTau) * (0.18 + body.clamp(0.0, 1.0) * 0.42);
      final ringEnv =
          math.exp(-t / ringTau) * (0.12 + ring.clamp(0.0, 1.0) * 0.55);
      final noiseEnv =
          math.exp(-t / noiseTau) * (0.2 + snares.clamp(0.0, 1.0) * 0.65);
      ...
      final noiseY = size.height * 0.72 -
          noiseEnv * size.height * 0.22 +
