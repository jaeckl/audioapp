...
class _MpcPadGridState extends State<MpcPadGrid> {
  ...
  int _pitchForPad(int index) =>
      (widget.pitchBase ?? 48) + widget.bankOffset + index;
  ...
  void _chokeSiblings(int index) {
    ...
    for (var i = 0; i < _padCount; i++) {
      if (i == index) continue;
      final sameCol =
          widget.chokeGroupByColumn && i % MpcPadGrid.columns == col;
  ...
  Future<void> _down(int index, double localY, double height) async {
    ...
    final velocity =
        velocityFromY(localY, height, curve: widget.velocityCurve).toDouble();
    try {
                  ...
                  children: [
                    for (var col = 0; col < MpcPadGrid.columns; col++) ...[
                      ...
                      Expanded(
                        child: _PadCell(
                          active: _held
                                  .contains(row * MpcPadGrid.columns + col) ||
                              widget.highlightedPitches.contains(
                                  _pitchForPad(row * MpcPadGrid.columns + col)),
                          onDown: (y, h) =>
                              _down(row * MpcPadGrid.columns + col, y, h),
                          onUp: () => _up(row * MpcPadGrid.columns + col),
                          onPointerDown: (pointer, local) => _onPadPointerDown(
                              row * MpcPadGrid.columns + col, pointer, local),
