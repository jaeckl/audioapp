...
abstract final class AutomationEditorMetrics {
  ...
  static double valueFromDy(double dy, double height) =>
      (1.0 - dy / height).clamp(0.0, 1.0);

  static double dyFromValue(double value, double height) =>
      height - value.clamp(0.0, 1.0) * height;

  static double dxFromBeat(double beat, double pixelsPerBeat) => beat * pixelsPerBeat;

  static const double minValueAxisHeight = 120;
  static const double maxValueAxisScale = 4.0;

  static double clampValueAxisHeight(double height, double viewportHeight) {
    final minH = viewportHeight > minValueAxisHeight ? viewportHeight : minValueAxisHeight;
