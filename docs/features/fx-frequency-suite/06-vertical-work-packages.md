...
- Processing functions use juce::dsp for shelf EQ (band 1/4) and juce::IIR::Coefficients for peak EQ (band 2/3)
...
**Canonical names used**: FourBandEqInstance, FourBandEqDeviceType, ffxBand\{1-4\}\{Freq,Gain,Q\}
...
**Contracts used**: FrequencyShifterInstance, normalizedToFrequency helper
...
**Forbidden files**: Engine-side files, existing Flutter files

**Canonical names used**: ffxCutoff, ffxResonance, ffxFilterMode, ffxBand\{1-4\}\{Freq,Gain,Q\}, ffxShift

**Contracts used**: DeviceSnapshot field names must match project_snapshot.dart (from WP-7)

**Dependencies**: WP-7 must define DeviceSnapshot fields (or WP-6 can use the known field names and WP-7 will match)

**Panel designs**:
- **FilterPanel**: `_dynamicsSinglePage()` layout with FilterPreview in preview box, 2 knob grid rows
  - Row 1: Cutoff, Resonance, FilterMode selector
  - Uses `_knobGridRow` with `_knob()` helper from dynamics pattern
- **FourBandEqPanel**: `_dynamicsSinglePage()` layout with FourBandEqPreview in preview box
  - 4 rows, one per band: Freq, Gain, Q knobs
- **FreqShifterPanel**: `_dynamicsSinglePage()` layout
  - Single row: Shift knob (centered, -2000 to +2000 Hz display)
  - Simple waveform preview or frequency visualization
...
**Acceptance criteria**:
- All three panels follow dynamics FX panel layout conventions
...
**Parallel**: Safe after WP-7 defines DeviceSnapshot fields, or can run in parallel if field names are agreed

---

## WP-7: Flutter Integration

**Behavior**: Wire the three new devices into all Flutter routing files.

**Assigned files**:
- MODIFY `app_flutter/lib/bridge/project_snapshot.dart`
...
1. **project_snapshot.dart**: Add 15 new fields to DeviceSnapshot with defaults, add fromMap parsing
2. **device_strip_slot.dart**: Add 3 switch cases routing to new panels, import frequency_fx_panels.dart
3. **device_strip_chrome.dart**: Add types to `_dynamicsTypes` set (so they get DynamicsInputPanel + DynamicsOutputPanel)
4. **device_strip_metrics.dart**: Add design widths, input/output widths
5. **device_strip_theme.dart**: Add accent colors and labels
6. **device_container_tabs.dart**: Add entries returning empty tab lists
7. **device_picker_sheet.dart**: Add "Frequency Effects" section header + 3 device entries

**Accent colors** (choose):
- Filter: `Color(0xFF5BC0EB)` (teal/cyan)
- 4-Band EQ: `Color(0xFF78C091)` (sage green)
- Frequency Shifter: `Color(0xFFC77DFF)` (purple/lavender)

**Design widths**: Use `dynamicsFxDesignWidth` (same as dynamics devices, ~216px)
...
**Parallel**: Sequential after WP-6
