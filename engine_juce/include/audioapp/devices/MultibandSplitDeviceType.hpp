...
/// Multiband crossover container (2/3/4 bands). One instance registered per bandCount.
class MultibandSplitDeviceType final : public IDeviceType {
public:
    explicit MultibandSplitDeviceType(int bandCount) noexcept;

    std::string typeId() const override;
    DeviceSlot createDefault(const std::string& deviceId) const override;
    DeviceParameterResult setParameter(DeviceSlot& slot, std::string_view parameterId,
