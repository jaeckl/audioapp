...
DeviceSlot ChainDeviceType::createDefault(const std::string& id) const {
    DeviceSlot s; s.id=id; s.config.typeId=typeId(); s.config.instance=ChainModel{};
    s.config.inputPanel=EmptyPanel{}; s.config.outputPanel=EmptyPanel{}; return s;
}
DeviceParameterResult ChainDeviceType::setParameter(DeviceSlot& s,std::string_view id,float v) const {
    auto& m=std::get<ChainModel>(s.config.instance);
    if(id=="chainMix") m.mix=std::clamp(v,0.f,1.f);
    else if(id=="chainGain") m.gain=std::clamp(v,0.f,2.f); else return {};
    return {.handled=true};
}
...
void ChainDeviceType::buildPlaybackNode(const DeviceSlot& s,const PlaybackBuildContext& c,DeviceNodePlayback& o) const {
    ...
    o.kind=DeviceNodeKind::Chain; o.params=ChainParams{p,m.mix,m.gain};
}
...
DeviceProcessor* ChainDeviceType::createProcessor(ProcessorArena&a) const{return a.emplace<ChainProcessor>();}
DeviceNodeKind ChainDeviceType::kind() const noexcept{return DeviceNodeKind::Chain;}
uint16_t ChainDeviceType::paramIdFromString(std::string_view n) const noexcept{return n=="chainMix"?0:n=="chainGain"?1:static_cast<uint16_t>(-1);}
std::string_view ChainDeviceType::paramIdToString(uint16_t i) const noexcept{return i==0?"chainMix":i==1?"chainGain":"";}
