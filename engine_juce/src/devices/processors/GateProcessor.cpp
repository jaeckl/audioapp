...
void GateProcessor::process(AudioBlock& block, ProcessContext& ctx) noexcept {
    auto p = std::get<GateParams>(*ctx.modulatedParams);
    ...
    processGateStereoBlock(block.channelL, block.channelR, block.numSamples, ctx.sampleRate, p, runtime);
