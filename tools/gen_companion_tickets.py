...
FEATURES: list[dict] = [
    ...
    },
    {
        "milestone": "milestone-06",
        "id": "06-03",
        "title": "Sampler on device strip",
        "parent": "US-06-03-sampler-device-on-strip.md",
        "status": "Todo",
        "ux": {
            "intent": "Sampler card shows loaded sound by name — confidence in assignment.",
            "layout": "Strip card: Sampler title, sample name, Choose sample button.",
            "states": [
                ("Unassigned", "No sample / placeholder"),
                ("Assigned", "Sample name prominent"),
            ],
            "copy": ["Sampler", "Choose sample", "No sample selected"],
            "ac": ["Name truncates gracefully", "Choose obvious"],
        },
        "ix": {
            "entry": ["Add sampler to strip", "Choose sample"],
            "map": [
                ("Add sampler", "Add device", "Card appears", "addDeviceToTrack"),
                ("Choose", "Button", "Library picker", "Assign sample ID"),
            ],
            "cancel": "Picker cancel → no change",
            "errors": [("Missing sample on play", "Documented toast", "No crash")],
            "demo": ["Add sampler → choose kick → name on card"],
            "ac": ["Return from library preserves strip context"],
        },
    },
    {
        "milestone": "milestone-06",
        "id": "06-04",
        "title": "MIDI triggers sample",
        "parent": "US-06-04-midi-triggers-sample.md",
        "status": "Todo",
        "ux": {
            "intent": "Playback moment — user hears drums/samples not test tone.",
            "layout": "No extra chrome during play; clip + playhead as M03.",
            "states": [("Playing sample", "Optional clip pulse")],
            "copy": "None",
            "ac": ["Clearly sample not oscillator"],
        },
        "ix": {
            "entry": ["Play with sampler + MIDI clip"],
            "map": [("Trigger", "Play", "Sample voices", "MIDI→sampler")],
            "cancel": "Stop",
            "errors": [("No sample", "Error on play attempt", "Silent fail NOT OK")],
            "demo": ["4-on-floor → Save → Load → Play"],
            "ac": ["Round-trip on device"],
        },
    },
    {
